...
export class CancelUpdateComponent implements OnInit {
  ...
  save(): void {
    this.isSaving = true;
    const cancel = this.createFromForm();
    if (cancel.id !== undefined) {
      this.subscribeToSaveResponse(this.cancelService.update(cancel));
    } else {
      this.subscribeToSaveResponse(this.cancelService.create(cancel));
    }
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<ICancel>>): void {
  ...
  protected updateForm(cancel: ICancel): void {
  ...
  protected createFromForm(): ICancel {
    return {
      ...new Cancel(),
