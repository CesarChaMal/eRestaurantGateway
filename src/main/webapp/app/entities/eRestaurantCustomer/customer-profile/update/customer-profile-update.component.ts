import { Component, OnInit } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { FormBuilder, Validators } from '@angular/forms';
import { ActivatedRoute } from '@angular/router';
import { Observable } from 'rxjs';
import { finalize } from 'rxjs/operators';

import { ICustomerProfile, CustomerProfile } from '../customer-profile.model';
import { CustomerProfileService } from '../service/customer-profile.service';
import { AlertError } from 'app/shared/alert/alert-error.model';
import { EventManager, EventWithContent } from 'app/core/util/event-manager.service';
import { DataUtils, FileLoadError } from 'app/core/util/data-util.service';

@Component({
  selector: 'jhi-customer-profile-update',
  templateUrl: './customer-profile-update.component.html',
})
export class CustomerProfileUpdateComponent implements OnInit {
  isSaving = false;

  editForm = this.fb.group({
    id: [null, [Validators.required, Validators.minLength(5)]],
    name: [null, [Validators.required, Validators.minLength(3)]],
    location: [null, [Validators.required, Validators.minLength(3)]],
    locationRange: [null, [Validators.required, Validators.minLength(3)]],
    referals: [],
  });

  constructor(
    protected dataUtils: DataUtils,
    protected eventManager: EventManager,
    protected customerProfileService: CustomerProfileService,
    protected activatedRoute: ActivatedRoute,
    protected fb: FormBuilder
  ) {}

  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ customerProfile }) => {
      this.updateForm(customerProfile);
    });
  }

  byteSize(base64String: string): string {
    return this.dataUtils.byteSize(base64String);
  }

  openFile(base64String: string, contentType: string | null | undefined): void {
    this.dataUtils.openFile(base64String, contentType);
  }

  setFileData(event: Event, field: string, isImage: boolean): void {
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe({
      error: (err: FileLoadError) =>
        this.eventManager.broadcast(
          new EventWithContent<AlertError>('eRestaurantGatewayApp.error', { ...err, key: 'error.file.' + err.key })
        ),
    });
  }

  previousState(): void {
    window.history.back();
  }

  save(): void {
    this.isSaving = true;
    const customerProfile = this.createFromForm();
    if (customerProfile.id !== undefined) {
      this.subscribeToSaveResponse(this.customerProfileService.update(customerProfile));
    } else {
      this.subscribeToSaveResponse(this.customerProfileService.create(customerProfile));
    }
  }

  protected subscribeToSaveResponse(result: Observable<HttpResponse<ICustomerProfile>>): void {
    result.pipe(finalize(() => this.onSaveFinalize())).subscribe({
      next: () => this.onSaveSuccess(),
      error: () => this.onSaveError(),
    });
  }

  protected onSaveSuccess(): void {
    this.previousState();
  }

  protected onSaveError(): void {
    // Api for inheritance.
  }

  protected onSaveFinalize(): void {
    this.isSaving = false;
  }

  protected updateForm(customerProfile: ICustomerProfile): void {
    this.editForm.patchValue({
      id: customerProfile.id,
      name: customerProfile.name,
      location: customerProfile.location,
      locationRange: customerProfile.locationRange,
      referals: customerProfile.referals,
    });
  }

  protected createFromForm(): ICustomerProfile {
    return {
      ...new CustomerProfile(),
      id: this.editForm.get(['id'])!.value,
      name: this.editForm.get(['name'])!.value,
      location: this.editForm.get(['location'])!.value,
      locationRange: this.editForm.get(['locationRange'])!.value,
      referals: this.editForm.get(['referals'])!.value,
    };
  }
}
