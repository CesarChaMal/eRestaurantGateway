...
export class PaymentService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/payments', 'erestaurantcustomer');
  ...
  update(payment: IPayment): Observable<EntityResponseType> {
    return this.http.put<IPayment>(`${this.resourceUrl}/${getPaymentIdentifier(payment) as string}`, payment, { observe: 'response' });
  }

  partialUpdate(payment: IPayment): Observable<EntityResponseType> {
    return this.http.patch<IPayment>(`${this.resourceUrl}/${getPaymentIdentifier(payment) as string}`, payment, { observe: 'response' });
  }
